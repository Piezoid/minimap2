				   ...
				   int8_t q, int8_t e, int8_t q2, int8_t noncan, int8_t specified_intron_score, int zdrop, int flag, ksw_extz_t *ez)
{
	...
	if (ksw_simd < 0) ksw_simd = x86_simd();
	if (ksw_simd & SIMD_SSE4_1)
		ksw_exts2_sse41(km, qlen, query, tlen, target, m, mat, q, e, q2, noncan, specified_intron_score, zdrop, flag, ez);
	else if (ksw_simd & SIMD_SSE2)
		ksw_exts2_sse2(km, qlen, query, tlen, target, m, mat, q, e, q2, noncan, specified_intron_score, zdrop, flag, ez);
	else abort();
}
#endif
